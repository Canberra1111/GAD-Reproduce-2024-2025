...
def load_data(data, prefix='data/'):
    ...
    if data == 'yelp':
        ...
        # 处理标签 - 兼容不同的键名 (Label vs label)
        if 'label' in data_file:
            labels = data_file['label'].flatten()
        elif 'Label' in data_file:
            labels = data_file['Label'].flatten()
        else:
            raise KeyError(f"No label found in YelpChi.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        
        # 处理特征 - 兼容不同的键名 (features vs Attributes)
        if 'features' in data_file:
            feat_data = data_file['features'].todense().A
        elif 'Attributes' in data_file:
            feat_data = data_file['Attributes'].todense().A
        else:
            raise KeyError(f"No features found in YelpChi.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        ...
        if 'label' in data_file:
            ...
            raise KeyError(f"No label found in Amazon.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        ...
        if 'features' in data_file:
            ...
            raise KeyError(f"No features found in Amazon.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        ...
        data_file = loadmat(prefix + 'questions.mat')
        ...
        if 'label' in data_file:
            ...
            labels = data_file['y'].flatten()
        else:
            raise KeyError(f"No label found in questions.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        
        # 处理特征
        if 'features' in data_file:
            feat_data = data_file['features'].todense().A if hasattr(data_file['features'], 'todense') else data_file['features']
        elif 'Attributes' in data_file:
            feat_data = data_file['Attributes'].todense().A if hasattr(data_file['Attributes'], 'todense') else data_file['Attributes']
        elif 'X' in data_file:
            feat_data = data_file['X'].todense().A if hasattr(data_file['X'], 'todense') else data_file['X']
        else:
            raise KeyError(f"No features found in questions.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        
        # load the preprocessed adj_lists
        with open(prefix + 'questions_homo_adjlists.pickle', 'rb') as file:
            homo = pickle.load(file)
        with open(prefix + 'questions_rel1_adjlists.pickle', 'rb') as file:
            relation1 = pickle.load(file)
        with open(prefix + 'questions_rel2_adjlists.pickle', 'rb') as file:
            relation2 = pickle.load(file)
        with open(prefix + 'questions_rel3_adjlists.pickle', 'rb') as file:
            relation3 = pickle.load(file)
    
    elif data == 'reddit':
        data_file = loadmat(prefix + 'Reddit.mat')
        ...
        if 'label' in data_file:
            ...
            raise KeyError(f"No label found in Reddit.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        ...
        if 'features' in data_file:
            ...
            raise KeyError(f"No features found in Reddit.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        
        # load the preprocessed adj_lists
        with open(prefix + 'reddit_homo_adjlists.pickle', 'rb') as file:
            homo = pickle.load(file)
        with open(prefix + 'reddit_rel1_adjlists.pickle', 'rb') as file:
            relation1 = pickle.load(file)
        with open(prefix + 'reddit_rel2_adjlists.pickle', 'rb') as file:
            relation2 = pickle.load(file)
        with open(prefix + 'reddit_rel3_adjlists.pickle', 'rb') as file:
            relation3 = pickle.load(file)
        ...
        if 'label' in data_file:
            ...
            raise KeyError(f"No label found in tfinance.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        ...
        if 'features' in data_file:
            ...
            raise KeyError(f"No features found in tfinance.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        
        # load the preprocessed adj_lists
        with open(prefix + 'tfinance_homo_adjlists.pickle', 'rb') as file:
            homo = pickle.load(file)
        with open(prefix + 'tfinance_rel1_adjlists.pickle', 'rb') as file:
            relation1 = pickle.load(file)
        with open(prefix + 'tfinance_rel2_adjlists.pickle', 'rb') as file:
            relation2 = pickle.load(file)
        with open(prefix + 'tfinance_rel3_adjlists.pickle', 'rb') as file:
            relation3 = pickle.load(file)
    
    elif data == 'tolokers':
        data_file = loadmat(prefix + 'tolokers.mat')
        ...
        if 'label' in data_file:
            ...
            raise KeyError(f"No label found in tolokers.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        ...
        if 'features' in data_file:
            ...
            raise KeyError(f"No features found in tolokers.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        
        # load the preprocessed adj_lists
        with open(prefix + 'tolokers_homo_adjlists.pickle', 'rb') as file:
            homo = pickle.load(file)
        with open(prefix + 'tolokers_rel1_adjlists.pickle', 'rb') as file:
            relation1 = pickle.load(file)
        with open(prefix + 'tolokers_rel2_adjlists.pickle', 'rb') as file:
            relation2 = pickle.load(file)
        with open(prefix + 'tolokers_rel3_adjlists.pickle', 'rb') as file:
            relation3 = pickle.load(file)
    
    elif data == 'weibo':
        data_file = loadmat(prefix + 'weibo.mat')
        ...
        if 'label' in data_file:
            ...
            raise KeyError(f"No label found in weibo.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        ...
        if 'features' in data_file:
            ...
            raise KeyError(f"No features found in weibo.mat. Available keys: {[k for k in data_file.keys() if not k.startswith('__')]}")
        
        # load the preprocessed adj_lists
        with open(prefix + 'weibo_homo_adjlists.pickle', 'rb') as file:
            homo = pickle.load(file)
        with open(prefix + 'weibo_rel1_adjlists.pickle', 'rb') as file:
            relation1 = pickle.load(file)
        with open(prefix + 'weibo_rel2_adjlists.pickle', 'rb') as file:
            relation2 = pickle.load(file)
        with open(prefix + 'weibo_rel3_adjlists.pickle', 'rb') as file:
            relation3 = pickle.load(file)
    
    else:
        raise ValueError(f"Dataset '{data}' not supported. Available datasets: yelp, amazon, questions, reddit, tfinance, tolokers, weibo")
