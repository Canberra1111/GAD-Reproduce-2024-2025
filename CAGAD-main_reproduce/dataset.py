    ...
    def __init__(self, name='tfinance', homo=True, anomaly_alpha=None, anomaly_std=None):
        ...
        if name == 'tfinance':
            ...
            if len(graph.ndata['label'].shape) > 1 and graph.ndata['label'].shape[1] > 1:
                # 如果是2D张量且有多个类别，使用argmax
                graph.ndata['label'] = graph.ndata['label'].argmax(1)
            else:
                # 如果已经是1D张量，直接使用
                graph.ndata['label'] = graph.ndata['label'].squeeze()
            ...
            if anomaly_alpha:
                ...
                if len(graph.ndata['label'].shape) > 1 and graph.ndata['label'].shape[1] > 1:
                    ...
                    label = graph.ndata['label'].squeeze()
                    anomaly_id = list((label == 1).nonzero().squeeze(1))
                    normal_id = list((label == 0).nonzero().squeeze(1))
